 ...
 */
public class DoomMakeExecuteWithArgsPanel extends JPanel
{
	...
	public DoomMakeExecuteWithArgsPanel()
	{
		this(new ScriptExecutionSettings());
	}
	
	public DoomMakeExecuteWithArgsPanel(ScriptExecutionSettings executionSettings)
	{
		...
		containerOf(this,
			node(BorderLayout.NORTH, utils.createForm(form(language.getInteger("wadscript.run.withargs.label.width")),
				utils.formField("doommake.run.withargs.stdin", standardInPathField),
				utils.formField("doommake.run.withargs.entrypoint", entryPointField),
				utils.formField("doommake.run.withargs.argfield", numArgsField)
			)),
